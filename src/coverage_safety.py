...
from instrumentation.specification import CoverageRobustnessSpec
from thermostat import Controller, controller_2rooms, controller_4rooms

logger = logging.getLogger("coverage_safety")
...
def main():
    ...
    ur_spec = CoverageRobustnessSpec(formula, controller)
    ...
    ur_best_cov = min((e.cost for e in ur_run.history), key=lambda c: c.remaining_states)
    ur_best_rob = min((e.cost for e in ur_run.history), key=lambda c: c.safety)

    soar_spec = CoverageRobustnessSpec(formula, controller)
    ...
    soar_best_cov = min((e.cost for e in soar_run.history), key=lambda c: c.remaining_states)
    soar_best_rob = min((e.cost for e in soar_run.history), key=lambda c: c.safety)

    n_branches = len(soar_spec.kripke.states)

    print(f"Function branches: {n_branches}")

    if ur_best_cov.remaining_states == 0:
        print(f"UR coverage achieved in: {len(ur_run.history)}")
    else:
        print(f"UR coverage not achieved in: {len(ur_run.history)}")

    print(f"UR minimum robustness: {ur_best_rob.safety}")

    if soar_best_cov.remaining_states == 0:
        print(f"SOAR coverage achieved in: {len(soar_run.history)}")
    else:
        print(f"SOAR coverage not achieved in: {len(soar_run.history)}")

    print(f"SOAR minimum robustness: {soar_best_rob.safety}")
